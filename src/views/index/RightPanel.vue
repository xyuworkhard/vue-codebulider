      ...
      <el-scrollbar class="right-scrollbar">
        ...
        >
          <el-form-item label="数据表">
            <el-input v-model="sheetName" style="pointer-events: none" />
          </el-form-item>
          <el-form-item
            label="字段名"
          ...
          <el-form-item
            v-if="
              activeData.__vModel__ !== undefined &&
                ...
                activeData.__config__.tag !== 'uploader' &&
                activeData.__config__.tagIcon !== 'code' &&
                activeData.__config__.tag !== 'grid'
            "
            label="默认值"
          >
            <el-input
          ...
          <el-form-item
            label="单据规则"
            v-if="activeData.__config__.tagIcon == 'code'"
          >
            <el-select
              v-model="dataRules"
              filterable
              placeholder="请选择"
              clearable
              @change="getCodeNumber"
            >
              <el-option
                v-for="item in dataRulesOption"
                :key="item.value"
                :label="item.label"
                :value="item.value"
              >
          ...
          >
            ...
            >
              <el-radio-group
                ...
                size="small"
              >
                <el-radio-button label="sourceData">
                  动态数据
                </el-radio-button>
                <el-radio-button label="customData">
                  静态数据
                </el-radio-button>
              </el-radio-group>
            </el-form-item>
            <template v-if="activeData.__config__.dataType === 'customData'">
              <draggable
            ...
            <template v-if="activeData.__config__.dataType === 'sourceData'">
              <!-- todo -->
              <el-form-item label="数据来源">
                ...
                >
                  ...
                  <el-option label="数据源" value="2"></el-option>
                </el-select>
              </el-form-item>
              <el-form-item label="数据选项" v-if="dataDictionaryFlag">
                <el-cascader
          ...
          <template
            v-if="
              ['el-cascader', 'el-table'].includes(activeData.__config__.tag) &&
                activeData.__config__.dataType != 'citySelect'
            "
          >
            <el-divider>选项</el-divider>
            <el-form-item
            ...
            <div v-if="activeData.__config__.dataType === 'static'">
              <template v-if="activeData.__config__.tag === 'el-table'">
                ...
                >
                  <div
                    v-for="(item,
                    index) in activeData.__config__.children.filter(
                      item => item.flag !== 'table_btnMark'
                    )"
                    :key="index"
                    class="select-item"
                  >
                    ...
                    <el-input
                      ...
                      size="small"
                      :value="item.prop"
                      @input="setTableValue(item, $event)"
                    />
                    <div
                      v-if="index != 0"
                      class="close-btn select-line-icon"
            ...
            <!-- 表格按钮 -->
            <template v-if="activeData.__config__.dataType === 'tableBtn'">
              <template
                v-if="
                  activeData.__config__.children.filter(
                    item => item.flag === 'table_btnMark'
                  ).length
                "
              >
                <div
                  v-for="(item, index) in activeData.__config__.children.filter(
                    item => item.flag === 'table_btnMark'
                  )[0].__config__.children"
                  :key="index"
                  class="select-item"
                >
                  <div class="select-line-icon option-drag">
                  ...
                  <el-input
                    ...
                    @input="setTableBtnValue(item, $event)"
                  />
                  <div
                    class="close-btn select-line-icon"
                    @click="delTableBtn(index)"
                  >
                    <i class="el-icon-remove-outline" />
                  </div>
                </div>
          ...
          <template v-if="activeData.__config__.dataType == 'citySelect'">
            <el-divider>选项</el-divider>
          </template>
          <el-form-item
...
export default {
  ...
  computed: {
    ...
    tableBtnOne() {
      return this.activeData.__config__.tag === "el-table"
        ? "表格数据"
        : "动态数据";
    },
    tableBtnTwo() {
  ...
  created() {
    let { dataBaseId, sheetName, dataRules } = sessionStorage;
    this.dataRulesOption = JSON.parse(dataRules);
    ...
    this.resDataDeal(
      ...
      resData => {
        this.sheetOptions = resData.map(item => item.f_column);
      }
    );
  },
  methods: {
    getDataSource(val) {
      this.$set(this.activeData.__config__, "dataOrigin", val);
      let { dataDictionary, dataSource } = sessionStorage;
      if (val == 1) {
        this.dataDictionaryFlag = true;
        this.dataOptions = this.getNewTreeData(JSON.parse(dataDictionary));
      } else {
        this.dataDictionaryFlag = false;
        let newArr = [];
        ...
        this.$set(this.activeData.__slot__, "options", newArr);
      }
    },
    getCaderValue(value) {
      let length = value.length > 0 ? value.length - 1 : 0;
      this.$set(this.activeData.__config__, "dictionaryCode", value[length]);
      this.resDataDeal(
        "/csce/learun/adms/dataitem/details",
        {
          data: value[length]
        },
        resData => {
          ...
          // this.activeData.__slot__.options = newArr;
          this.$set(this.activeData.__slot__, "options", newArr);
        }
      );
    },
    ...
    getCodeNumber(code) {
      this.resDataDeal(
        "/csce/learun/adms/coderule/code",
        { data: code },
        resData => {
          this.activeData.__config__.defaultValue = resData;
          this.activeData.__config__.codeValue = code;
        }
      );
    },
    async resDataDeal(url, params, callback) {
    ...
    addTableBtnItem() {
      let lastIndex = this.activeData.__config__.children.length - 1;
      let btnFlag = this.activeData.__config__.children[lastIndex].flag;
      if (btnFlag) {
        this.activeData.__config__.children[lastIndex].__config__.children.push(
          {
            __config__: {
              label: "按钮",
              tag: "el-button",
    ...
    addTableItem() {
      ...
      let btnFlag = this.activeData.__config__.children[lastIndex].flag;
      if (btnFlag) {
        this.activeData.__config__.children.splice(-1, 0, {
          __config__: {
            layout: "raw",
            tag: "el-table-column",
            renderKey: +new Date()
    ...
    setTableValue(item, val) {
      item.prop = val;
    },
    setTableBtnValue(item, val) {
      item.__slot__.default = val;
    },
    delTableBtn(val) {
      let lastIndex = this.activeData.__config__.children.length - 1;
      this.activeData.__config__.children[lastIndex].__config__.children.splice(
        val,
        1
      );
      let btnFlag =
        this.activeData.__config__.children[lastIndex].flag == "table_btnMark";
      if (btnFlag) {
        let lengths = this.activeData.__config__.children[lastIndex].__config__
          .children.length;
        lengths ? "" : this.activeData.__config__.children.splice(lastIndex, 1);
      }
    },
    setDefaultValue(val) {
