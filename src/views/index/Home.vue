<template>
  <el-card class="homecard">
    <div class="container" v-if="isFresh">
...
export default {
  ...
  watch: {
    $route: {
      handler(val) {
        ...
        this.$nextTick(() => {
          this.isFresh = true;
          let dataFields = JSON.parse(sessionStorage.getItem("dataFields"));
          this.drawingList = [];
          dataFields.forEach(ele => {
            ...
            dataConfig.__config__.renderKey = `${
              dataConfig.__config__.formId
            }${+new Date()}`;
            dataConfig.__vModel__ = ele.value;
            dataConfig.__config__.label = ele.label;
            this.drawingList.push(dataConfig);
          });
  ...
  created() {
    this.$emit("changeValue", 3, false, true);
  ...
  methods: {
    ...
    setRespData(component, respData) {
      ...
      const data = dataPath.split(".").reduce((pre, item) => {
        return pre[item];
      }, respData);
    ...
    fetchData(component) {
      const { dataType, method, url, tagIcon } = component.__config__;
    ...
    generateCode() {
      ...
      //todo 复制
      const fieldArr = this.formData.fields;
      const cityType = fieldArr.find(item => {
        const {
          __config__: { dataType }
        } = item;
        return dataType == "citySelect";
      });
      if (cityType) {
        const cityimport =
          "\n import { regionData } from 'element-china-area-data'; \n";
        script = script.replace(/<script>/, "<script>" + cityimport);
      }
    ...
    tagChange(newTag) {
      ...
      if (config.dataType) {
        this.activeData.__config__.dataType = config.dataType;
      }
      if (
...
.homecard.el-card .el-card__body {
  padding: 0;
}
</style>
