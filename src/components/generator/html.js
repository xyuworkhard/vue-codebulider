...
const tags = {
  ...
  'el-table': el => {
    ...
    const tableData = `${el.__vModel__}TableData`;
    let datasArr = el.__config__.children.filter(item => item.flag !== "table_btnMark");
    let hasBtns = el.__config__.children.filter(item => item.flag === "table_btnMark").length;
    let dataTag = "";
    if (hasBtns) {
      let btnsArr = el.__config__.children.filter(item => item.flag === "table_btnMark")[0].__config__.children;
      dataTag = ` 
     <el-table-column label="操作">
      <template slot-scope="scope">
       ${btnsArr.map(item => `<el-button type="primary">${item.__slot__.default}</el-button>`)}
      </template>
    </el-table-column>`
    } else {
      dataTag = ""
    ...
    let tableTag = ` 
    <${tag} :data="${tableData}" border>
     ${datasArr.map(item => `<el-table-column prop="${item.prop}" label="${item.label}" ></el-table-column>`)}` + dataTag + `</${tag}>`
    return tableTag.split(',').join('')
  },
  'el-radio-group': el => {
  ...
  uploader: el => {
    const tag = el.__config__.tag
    const keyValue = `:key-value="${confGlobal.formModel}.${el.__vModel__}"`
    const vModel = `v-model="${confGlobal.formModel}.${el.__vModel__}"`
    const fileType = `:file-type="'${el.__vModel__}'"`
    const companyName = `:company-name="'webCode'"`
    return `<${tag} ${vModel} ${keyValue} ${fileType} ${companyName}></${tag}>`
  },
  grid: el => {
    // : tableData = "tableData" @sendBackData="getBackData" : gridConfig = "gridConfig"
    const tag = el.__config__.tag
    const vModel = `v-model="${confGlobal.formModel}.${el.__vModel__}"`
    const tableData = `:tableData = "table${el.__vModel__}Data"`
    const sendBackData = `@sendBackData="get${el.__vModel__}BackData"`
    const gridConfig = `:gridConfig="grid${el.__vModel__}Config"`
    return `<${tag} ${vModel} ${tableData} ${sendBackData} ${gridConfig}></${tag}>`
  },
